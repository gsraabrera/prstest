...
class AddPage extends RunnerPage
{
	...
	public function getContolMapData( $fName, $recId, &$data, $pageFields ) {
		...
		if( $preload !== false ) {
			...
			//	select first value in a lookup control. 
			// It hasn't worked since 2015. Started giving erros with PHP 8.3 (array instead of scalar value)
			// Fixed, but commented out to not change existing behavior
			/*
			if( !$data[ $fName ] && $preload["vals"] ) {
				$data[ $fName ] = $preload["vals"][0]["linkValue"];
			}
			*/
		}
