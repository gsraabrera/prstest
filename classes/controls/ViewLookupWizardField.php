...
class ViewLookupWizardField extends ViewControl
{
	...
	protected function getLookupValues( $value )
	{
		...
		// This function uses isset() instead of array_key_exists() because $value and resolvedLookupValues keys 
		// come from different tables and may have compatible but different types: float/int, string/int etc
		// $arr[10] = 'aaa'
		// isset( arr[float(10)] ) - true
		// array_key_exists( float(10), arr ) - false
		...
		$unresolved = array();
		foreach( $values as $v ) {
			if( !isset( $this->resolvedLookupValues[ $v ] ) ) {
				$unresolved[] = $v;
			}
		}
	...
	public function showDBValue( &$data, $keylink, $html = true )
	{
		...
		foreach( $lookupValues as $lv )
		{
			...
			if( $this->pSet->getViewFormat($this->field) == FORMAT_CUSTOM ) {
			...
			$outputValue = $this->localControlsContainer->showDBValue( $this->field, $localData, $keylink, $passValue, $html );
