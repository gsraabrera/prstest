...
class DashboardPage extends RunnerPage
{
	...
	function __construct(&$params)
	{
		...
		if( $params['mode'] == 'dashsnippet' || $params['mode'] == 'dashwidget' ) {
			
			foreach( $this->pSet->getDashboardElements() as $key => $elem ) {
				if ( $elem["type"] == DASHBOARD_SNIPPET && $elem["elementName"] == $this->elementName ) {
					echo printJSON( $this->showDashboardSnippet( $elem ) );
					exit();
				}
				if ( $elem["type"] == DASHBOARD_WIDGET && $elem["elementName"] == $this->elementName ) {
					echo printJSON( $this->showDashboardWidget( $elem ) );
					exit();
				}
			}
	...
	protected function callSnippet( $dashElem ) {
		...
		if( $globalEvents->dashSnippetExists( $dashElem['snippetId'] ) ) {
			...
			$snippetData["body"] = callDashboardSnippet( $funcName, $icon, $header, $tab );
		}
		$snippetData["header"] = $header;
		$snippetData["icon"] = $icon;
		$snippetData["tab"] = $tab ? $tab : $header;
		return $snippetData;
	}

	protected function showDashboardSnippet( $dashElem ) {
		$snippetData = $this->callSnippet( $dashElem );
		$iconHtml = getIconHTML( $snippetData["icon"] );
		$style = $dashElem["item"]["snippetStyle"];
	
		if( !$style || $style == 'classic' ) {
			$header = $snippetData["header"];
			$footer = '';
			$html = $snippetData["body"];
		} else {
			$header = '';
			$footer = '';
			$html = $this->getSnippetHtml( $dashElem, $snippetData );
		}
		return array(
  			'headerCont' => $header,
			'html' => $html,
			'footerCont' => $footer,
			'iconHtml' => $iconHtml,
			'idStartFrom' => $this->flyId,
			'success' => true,
			'additionalJS' => array(),
			'CSSFiles' => array(),
			'pagesData' => array( $this->id => array( 'pageTitle' => $snippetData["tab"] ) )
		);
	}

	protected function renderWidget( $dashElem ) {
		
		$this->xt->load_template( $this->templatefile );
		
		$bodyTag = 'widget_body_' . $dashElem['elementName'];
		$headerTag = 'widget_head_' . $dashElem['elementName'];

		$this->xt->assign( $headerTag, true );
		$this->xt->assign( $bodyTag, true );
		$header = $this->xt->fetch_loaded( $headerTag );
		$body = $this->xt->fetch_loaded( $bodyTag );
		$this->xt->assign( $headerTag, false );
		$this->xt->assign( $bodyTag, false );

		return array( 
			'header' => $header,
			'body' => $body,
			'icon' => $icon,
			'tab' => GetMLString( $dashElem['item']['tabLabel'] )
		);
	}

	protected function showDashboardWidget( $dashElem ) {
		
		$snippetData = $this->renderWidget( $dashElem );
		$iconHtml = getIconHTML( $snippetData["icon"] );
		$style = $dashElem["item"]["snippetStyle"];
		...
		return array(
			...
			'additionalJS' => array(),
			'CSSFiles' => array(),
			'pagesData' => array( $this->id => array( 
				'pageTitle' => $snippetData["tab"],
				'buttons' => $dashElem['widgetButtons'],
	...
	protected function fillElementPermissions() {
		foreach( $this->pSet->getDashboardElements() as $key => $elem )
		{
			...
			elseif( $elem['type'] == DASHBOARD_DETAILS)
			{
				...
				$permissions = true;
			} elseif( $elem['type'] == DASHBOARD_WIDGET ) {
				$permissions = true;
			}
	...
	protected function buildJsTableSettings( $table, $pSet ) {
		...
		foreach( $this->pSet->getDashboardElements() as $key => $elem )
		{
			...
			if( $elem['type'] == DASHBOARD_RECORD ) {
				$newElem['tabLabels'] = array(
					'add' => GetMLString( $elem['item']['addLabel'] ),
					'edit' => GetMLString( $elem['item']['editLabel'] ),
					'view' => GetMLString( $elem['item']['viewLabel'] ),
				);
			}
