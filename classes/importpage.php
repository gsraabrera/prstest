...
class ImportPage extends RunnerPage
{
	...
	protected function buildJsTableSettings( $table, $pSet ) {
		...
		$settings["importFieldsLabels"] = $this->getImportfieldsLabels();
	...
	protected function callBeforeInsert( &$rawvalues, &$fieldsValuesData, &$errorMessage ) {
		...
		if( $this->eventsObject->BeforeInsert($rawvalues, $fieldsValuesData, $this, $message ) === false ) {
			//	.NET conversion quirk requires direct assignment of byref variable
		...
		$isNotLogFile = true, $lineBreak = "<br>", $errorMessages = array(), $unprocessedData = array() )
	{
		...
		{
			$reportText .= mlang_message('IMPORT_INTO') . " " . $this->pSet->getOriginalTableName() . $lineBreak.
