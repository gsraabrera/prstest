...
class ListPage_Dashboard extends ListPage_Embed
{
	...
	*/
	...
	function commonAssign()
	{
		...
		if( $this->dashElementData['headerControls'] ) {
			//	don't show records per page
	...
	function isAllowShowHideFields() {
		return !$this->dashElementData['headerControls'] && $this->pSet->isAllowShowHideFields();
	}

	protected function reorderFieldsFeatureEnabled() {
		return !$this->dashElementData['headerControls'] && $this->pSet->isAllowFieldsReordering();
	}

	protected function getColumnsToHide() {
		if( $this->dashElementData['headerControls'] ) {
			return parent::getColumnsToHide();
		}
