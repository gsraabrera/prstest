...
class Security
{
	...
	static function processLogoutRequest()
	{
		...
		//	restore global page settings, which pages are available and which are not
		global $originalGlobalTableSettings, $runnerTableSettings;
		$runnerTableSettings[ GLOBAL_PAGES ] = $originalGlobalTableSettings;
	 ...
	 */
	protected static function getStaticGroupId() {
		$groupId = storageGet( "GroupID" );
		if( !$groupId ) {
			return null;
		}
		$permissions =& ProjectSettings::staticPermissions();
		if( array_key_exists( $groupId, ProjectSettings::staticPermissions() ) ) {
			return $groupId;
		}
		return '<Default>';
	}
	...
	public static function getUserGroupIds()
	{
		...
		if( !Security::dynamicPermissions() ) {
			//	static permissions
			$groupId = Security::getStaticGroupId();
			if( $groupId === null ) {
				return array();
			}
	...
	*  For AD it is <provider code><username>
	...
	public static function _staticRestrictedPages( $table ) {
		$permissions =& ProjectSettings::staticPermissions();
		$group = Security::getStaticGroupId();
		if( !$group ) {
			$group = '<Default>';
		}
	...
	public static function generateAndSendTwoFactorCode( $method ) {
		...
		if( $destination["method"] == TWOFACTOR_APP ) {
			...
			storageSet( "twoFactorCode", array( "method" => $method, "attempts" => 0 ) );
			return $destination;
		}
		//	generate 2f code
		...
		storageSet( "twoFactorCode", array( "method" => $method, "code" => $code, "sentAt" => time(), "attempts" => 0 ) );
