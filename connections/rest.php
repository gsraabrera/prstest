...
class RestConnection {
	...
	var $oauthType = oauthCredPASSWORD;
	...
	function __construct( $params ) {
		...
		if( isset( $params["oauthType"] ) ) {
			$this->oauthType = $params["oauthType"];
		} else {
			$this->clientCredentials = $params["clientCredentials"];
			$this->oauthType = $this->clientCredentials
				? oauthCredCLIENT
				: oauthCredPASSWORD;
		}
	...
	function oauthTokenRequestParams() {
		...
		switch( $this->oauthType ) {
			...
			case oauthCredSERVICEACC: {
				$now = time();
				$assertion = $this->makeGoogleServiceAssertion();
				return array(
					"grant_type" => "urn:ietf:params:oauth:grant-type:jwt-bearer",
					"assertion" => $this->makeGoogleServiceJwt( $assertion ),
				);
			}
	...
	function makeGoogleServiceJwt( $payload ) {
		$header64 = base64_encode_url( runner_json_encode( array('typ' => 'JWT', 'alg' => 'RS256') ) );
		$payload64 = base64_encode_url( runner_json_encode( $payload) );
		$jwtSecret = $this->private_key;
		$signature = hash_rs256($header64 . "." . $payload64, $jwtSecret );
		$base64UrlSignature = base64_encode_url_binary( $signature );
		return $header64 . "." . $payload64 . "." . $base64UrlSignature;
	}

	function makeGoogleServiceAssertion() {
		$skew = 60;
		$expiry = 3600;
		$now = time();
	...
	protected function createOauthTokenRequest( $params ) {
		...
		if( $this->oauthType != oauthCredSERVICEACC ) {
