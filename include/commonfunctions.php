...
function createStorageObject( $providerType, $additionalParams ) {
	...
	if( $providerType == stpAZUREBLOB ) {
		$params["accountName"] = ProjectSettings::azureBlobName();
		$params["accountKey"] = ProjectSettings::azureBlobKey();
		$params["container"] = ProjectSettings::azureBlobContainer();
		$params = array_merge( $params, $additionalParams );
		return new AzureBlobFileSystem( $params );
	}
}
...
function getStorageProvider( $pSet, $field ) {
	...
	if( $providerType == stpDISK) {
		...
		$params["path"] = $pSet->azureBlobPath( $field );
