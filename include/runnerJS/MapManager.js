	...
	addMarkerClickEvent: function( marker, markerData, markerAsEditLink ) { 
		...
		if ( markerAsEditLink && this.showEditInPopup ) {
			...
			this.providerMap.addOnMarkerClickHandler( marker, function() {				
				$("#editLink" + markerData.recId).trigger("click");
			});
...
Runner.GmapLoader = function() {
	...
	this.loadGMapScript = function( APIkey ) {			
		...
		script.src = "https://maps.googleapis.com/maps/api/js?" + [ keyParam, langStr, "loading=async&callback=Runner.globalGmapLoader.mapsLoaded&libraries=visualization,geometry,marker" ]
			.filter( _ => !!_ ).join( '&' );
		
		/*
		if ( APIkey != '' ) {
			script.src = location.protocol + "//maps.googleapis.com/maps/api/js?key=" + APIkey + "&" + langStr + "&callback=Runner.globalGmapLoader.mapsLoaded&libraries=visualization,geometry";
		} else {
			script.src = location.protocol + "//maps.googleapis.com/maps/api/js?" + langStr + "&callback=Runner.globalGmapLoader.mapsLoaded&libraries=visualization,geometry";
		}
		*/
...
(function() {
	...
	if ( window.settings['global'].mapProvider === Runner.controls.constants.GOOGLE_MAPS && !window.settings['global'].staticMapsOnly ) {
		Runner.globalGmapLoader.loadGMapScript( window.settings['global'].mapsApiCode );
		return;	
	} 
