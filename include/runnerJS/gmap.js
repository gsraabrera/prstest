	...
	setMarkerByCoords: function( markerData, mapData ) {
		...
		if ( this.mapIsHeatmap( mapData ) ) {
			...
			if ( markerData.mapIcon ) {
				...
				content.src = settings.global["webRootPath"] + markerData.mapIcon;
				marker.append( content );
			} else if( markerData.currentLocationMarker ) {
				marker.append( new google.maps.marker.PinElement({
		...
		if ( mapData.clustering && marker ) {
			...
			mapData.addedCoordinates[ this.coordToString( marker.position ) ] = true;
		}			
	},
...
//		google.maps.event.addDomListenerOnce( map, "tilesloaded", handler );
		map.addListener( "tilesloaded", handler );
	},
	...
	setMarkerActive: function( marker, customIcon ) {
		...
		marker.children[0]?.classList.add( 'marker-bounce' );
		setTimeout( function() { 
			//marker.setAnimation( null );
			marker.children[0]?.classList.remove( 'marker-bounce' );
...
//			marker.setAnimation( null );
	...
	initializeClusterMarkers: function( mapData ) {
		return new markerClusterer.MarkerClusterer( { 
			map: mapData.map, 
			markers: mapData.clusterMarkers, 
			imagePath: settings.global["webRootPath"] + 'images/m' 
		} );
	},
